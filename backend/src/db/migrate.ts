...
async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    console.log(`  Connected to PostgreSQL: ${result[0]?.db ?? 'unknown'}`);
    ...
    try {
      ...
      console.log(`  Users table: accessible (${users.length} sample rows)`);
    } catch {
      console.log('  Users table not found — run `npx drizzle-kit push` to create schema');
    }
