/**
 * Raw CM360 tool catalog, shared by the backend Claude executor and the
 * @adtraffic/mcp server. Credential-free — no feature-flag gating or backend
 * imports live here (those stay in backend/src/claude/tool-definitions.ts).
 *
 * 70 CM360 tools: reporting + floodlight + pacing + user & role management.
 */

/** Minimal structural shape; matches Anthropic.Tool's fields we rely on. */
export interface CM360ToolDefinition {
  name: string;
  description: string;
  input_schema: { type: 'object'; properties?: Record<string, unknown>; required?: string[] };
}

export const CM360_TOOLS: CM360ToolDefinition[] = [
  {
